...
#Energy Bill without P2P trading -> only interaction with grid
...
t_B_P2G_cons=as.data.frame(t(B_P2G_cons))
plotgrids = list()
i = 1
for(i in 1) {
  p_title = ggdraw() + 
    draw_label("Consumer Enenergy Bills without Micro-grid",
               size     = 24,
               fontface = "bold")
...
ggsave("output/actual/graphs/P2G_cons.png", height = 16.534, width = 23.384, bg="transparent")
...
#Energy Bill with P2P trading -> use Prcing Mechanism
...
B_P2G_prod = as.data.frame(matrix(NA, ncol=length(size), nrow=(ncol(cons)-1)))
i=2
s=1
for (s in 1:length(size)){
  for (i in 2:ncol(cons)){
...
P2G_cons = data.frame( "ID"             = substring(files_c, 15, 17),
...
plot_grid(ptitle, p, p1, ncol = 1, rel_heights = c(0.15, 1, 1), align = 'v')
ggsave("output/actual/graphs/energybills_cons.png",
       height = 8.267, width = 23.384, bg="transparent")
...
ggsave("output/actual/graphs/P2P_energy_sharing_Dec21.png", height = 16.534, width = 23.384, bg="transparent")
...
p = ggplot(df, aes(x = id, y=value, color=type)) +
  ...
  scale_y_continuous(limits = c(0.123, 0.314)) +
  ylab("Price (€/kWh)") +
...
ggsave("output/actual/graphs/price.png", height = 16.534, width = 23.384, bg="transparent")
